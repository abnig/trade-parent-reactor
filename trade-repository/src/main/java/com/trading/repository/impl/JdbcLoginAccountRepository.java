...
public class JdbcLoginAccountRepository implements LoginAccountRepository {
    ...
    public Optional<LoginAccount> findByUsername(String username) {
        var accounts = jdbc.query("SELECT id, username, password, enabled, account_non_expired, account_non_locked, credentials_non_expired, credential_version FROM users WHERE username = ?",
            (rs, row) -> new LoginAccount(rs.getLong("id"), rs.getString("username"), rs.getString("password"),
                ...
                rs.getBoolean("credentials_non_expired"), java.util.Set.of(), rs.getLong("credential_version")), username);
        ...
        return Optional.of(new LoginAccount(account.id(), account.username(), account.passwordHash(), account.enabled(),
                account.accountNonExpired(), account.accountNonLocked(), account.credentialsNonExpired(), roles, account.credentialVersion()));
    }
}
